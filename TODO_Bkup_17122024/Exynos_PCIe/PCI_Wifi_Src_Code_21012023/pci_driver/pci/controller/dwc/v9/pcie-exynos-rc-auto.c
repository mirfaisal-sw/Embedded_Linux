...
#if IS_ENABLED(CONFIG_PCI_EXYNOS_IOV)
#include "pcie-exynos-vf-auto.h"
#include "../../pci.h"
#endif
...
static void exynos_pcie_send_pme_turn_off(struct exynos_pcie *exynos_pcie);
...
static int exynos_pcie_wr_other_conf(struct pcie_port *pp,
		struct pci_bus *bus, u32 devfn, int where, int size, u32 val);
static void exynos_pcie_prog_viewport_cfg(struct pcie_port *pp, u32 busdev,
		u32 type);
static void exynos_pcie_prog_viewport_mem_outbound(struct pcie_port *pp);
static int exynos_pcie_write_other(struct exynos_pcie *exynos_pcie,
	void __iomem *addr, int size, u32 val);
static int exynos_pcie_read_other(struct exynos_pcie *exynos_pcie,
	void __iomem *addr, int size, u32 *val);
static int exynos_pcie_link_up(struct dw_pcie *pci);
#if IS_ENABLED(CONFIG_PCI_EXYNOS_IOV)
static void exynos_pcie_prog_viewport_mem_outbound_for_vf(struct pcie_port *pp,
	int vf_num, u64 base_addr, u64 target_addr, u32 size, u64 vf_bar_size);
#endif
...
void exynos_pcie_register_dump(int ch_num)
{
	...
	dev_info(pci->dev, "\n");
	dev_info(pci->dev, "PHY PMA PLL MON : 0x%08x\n",
			exynos_phy_read(exynos_pcie, 0x4D4));
	dev_info(pci->dev, "PHY PMA CDR LOCK : 0x%08x\n",
			exynos_phy_read(exynos_pcie, 0xCA4));

	/* PHY PCS : 0x0 ~ 0x200 */
	dev_info(pci->dev, "Print PHY PCS region...\n");
	for (i = 0; i < 0x200; i += 4) {
		dev_info(pci->dev, "PHY PCS 0x%04x : 0x%08x\n",
...
static int chk_epmem_outofbar_access(struct exynos_pcie *exynos_pcie)
{
	u32 val;
	int test_result = 0;
	struct dw_pcie *pci = exynos_pcie->pci;
	...
	if (exynos_pcie_rd_other_conf(pp, ep_pci_bus, 0, PCI_VENDOR_ID, 4,
		&val) == 0) {
...
static int chk_hard_access(struct exynos_pcie *exynos_pcie,
				void __iomem *check_mem)
{
	...
	while (count < 1000000) {
		for (i = 0; i < 10; i++) {
			ret = exynos_pcie_read_other(exynos_pcie,
 ...
 */
static int get_pcie_link_info(struct exynos_pcie *exynos_pcie)
{
	...
	if (exynos_pcie->state == STATE_LINK_UP) {
		...
		if (ep_pci_bus == NULL) {
			...
			if (exynos_pcie_rd_other_conf(pp, ep_pci_bus, 0,
		...
		for (i = PCI_STD_RESOURCES; i <= PCI_STD_RESOURCE_END; i++) {
			...
			if (flags > 0) {
				...
				sprintf(sInfo, "BAR %d: [mem 0x%llx-0x%llx",
					i, start, end);
			...
			for (j = 1; j <= num_vfs; j++) {
				...
				for (i = PCI_IOV_RESOURCES;
					...
					if (flags > 0) {
						size =
							ep_pci_dev[PCIE_PF]->
							sriov->barsz[i - PCI_IOV_RESOURCES];
						...
						sprintf(sInfo, "VF%d BAR %d: [mem 0x%llx-0x%llx ",
							...
							start + (size * j) - 1);
						if (flags & IORESOURCE_MEM_64) {
							dev_info(pci->dev, "%s 64bit]\n", sInfo);
						} else {
							dev_info(pci->dev, "%s 32bit]\n", sInfo);
						}
					}
				}
			}
 ...
 */
static struct pci_dev *exynos_pcie_get_pci_dev
	(struct exynos_pcie *exynos_pcie, int bus_num)
{
	...
	dev_info(pci->dev, "[%s] Vendor: 0x%x Device: 0x%x\n",
		__func__, val & ID_MASK, (val >> 16) & ID_MASK);
 ...
 * @noret
 */
static void print_pci_dev_info(struct pci_dev *pci_dev)
{
	if (pci_dev != NULL) {
		dev_info(NULL, "###############################\n");
 ...
 */
static irqreturn_t exynos_pcie_msi_irq_handler_vm(int irq, void *arg)
{
 ...
 */
static int sriov_request_msi(struct exynos_pcie *exynos_pcie,
	struct pci_dev *pci_dev, int vf_num, int int_num)
{
	...
	if (!exynos_pcie->sriov_msi_enable[vf_num]) {
		if (vf_num == PCIE_PF) {
			ret = pci_alloc_irq_vectors_affinity(pci_dev, int_num,
					int_num, PCI_IRQ_MSI, NULL);
			if (ret < 0) {
				dev_err(pci->dev,
		...
		if (vf_num > 0) {
			ret = devm_request_irq(pci->dev,
					exynos_pcie->msi_irq[vf_num-1],
					exynos_pcie_msi_irq_handler_vm,
 ...
 * @retval{-, int, 0, 0/-1, -1}
 */
static int sriov_create_vf(struct exynos_pcie *exynos_pcie, int num_vfs)
{
	u32 val, devfn;
	...
	if (exynos_pcie->state == STATE_LINK_UP) {
		...
		if (!exynos_pcie->sriov_supported) {
			exynos_pcie->ep_pci_dev[PCIE_PF] =
				exynos_pcie_get_pci_dev(exynos_pcie, 1);
		...
		for (i = PCIE_PF; i <= num_vfs; i++) {
			...
			if (exynos_pcie->use_msi) {
				...
				irq_num = sriov_request_msi(exynos_pcie,
				...
				reg = exynos_pcie->ep_pci_dev[i]->msi_cap +
					PCI_MSI_FLAGS;
				exynos_pcie_rd_other_conf(pp,
					exynos_pcie->ep_pci_dev[PCIE_PF]->bus,
					devfn, reg, 2, &val);

				val |= PCI_MSI_FLAGS_ENABLE;

				exynos_pcie_wr_other_conf(pp,
					exynos_pcie->ep_pci_dev[PCIE_PF]->bus,
					devfn, reg, 2, val);
			}

			print_pci_dev_info(exynos_pcie->ep_pci_dev[i]);
			...
			for (j = PCI_IOV_RESOURCES;
				...
				flags =
					exynos_pcie->ep_pci_dev[PCIE_PF]->
							resource[j].flags;
				if (flags > 0) {
					vf_bar_size = exynos_pcie->ep_pci_dev[PCIE_PF]->
							sriov->barsz[j - PCI_IOV_RESOURCES];
				}
			}
			exynos_pcie_prog_viewport_mem_outbound_for_vf(pp, i,
				exynos_pcie->mem_base, exynos_pcie->mem_bus_addr,
				PCIE_VF_OB_SIZE, vf_bar_size);

		}
		/* Update PMEM state LINK UP */
		exynos_vf_request_vlink_send(PF_LINKUP);
		...
		const char *buf, size_t count)
{
	...
	case 3:
		get_pcie_link_info(exynos_pcie);
		break;
	...
	case 9:
		if (exynos_pcie->state == STATE_LINK_UP) {
			...
			queue_work(exynos_pcie->pcie_wq, &exynos_pcie->hardaccess_work.work);
		} else {
	...
	case 13:
		if (exynos_pcie->state == STATE_LINK_UP)
			chk_epmem_outofbar_access(exynos_pcie);
		else
			dev_warn(dev, "Link up is not established\n");
		break;
	...
	case 14:
#if IS_ENABLED(CONFIG_PCI_EXYNOS_IOV)
		sriov_create_vf(exynos_pcie, exynos_pcie->num_vfs);
#else
...
static int exynos_pcie_clock_get(struct pcie_port *pp)
{
	...
	for (i = 0; i < total_clk_num; i++) {
		if (i < exynos_pcie->pcie_clk_num) {
			...
			if (phy_count < 0) {
				dev_err(pci->dev, "phy_count value is negative\n");
				return -EINVAL;
			}
...
static void exynos_pcie_setup_rc(struct pcie_port *pp)
{
	...
	/* RC BAR disable */
	exynos_pcie_rd_own_conf(pp, DBI_SHADOW_BASE_ADDR + PCI_BASE_ADDRESS_0,
		4, &val);
	dev_info(pci->dev, "[%s] Before RC BAR0 = 0x%x\n", __func__, val);
	val &= ~(0x1);
	exynos_pcie_wr_own_conf(pp, DBI_SHADOW_BASE_ADDR + PCI_BASE_ADDRESS_0,
		4, val);
	dev_info(pci->dev, "[%s] After RC BAR0 = 0x%x\n", __func__, val);

	exynos_pcie_rd_own_conf(pp, DBI_SHADOW_BASE_ADDR + PCI_ROM_ADDRESS,
		4, &val);
	dev_info(pci->dev, "[%s] Before RC Expansion ROM = 0x%x\n",
		__func__, val);
	val &= ~(PCI_ROM_ADDRESS_ENABLE);
	exynos_pcie_wr_own_conf(pp, DBI_SHADOW_BASE_ADDR + PCI_ROM_ADDRESS,
		4, val);
	...
	exynos_pcie_wr_own_conf(pp, PCIE_L1_SUBSTATES_OFF,
	4, PCIE_L1_SUB_VAL);
	...
	exynos_pcie_rd_own_conf(pp, pcie_cap_off + PCI_EXP_LNKCTL2,
								4, &val);
	val &= ~PCI_EXP_LNKCTL2_TLS;
	val |= exynos_pcie->max_link_speed;
	exynos_pcie_wr_own_conf(pp, pcie_cap_off + PCI_EXP_LNKCTL2,
								4, val);
	...
	/* Equalization off */
	exynos_pcie_rd_own_conf(pp, PCIE_GEN3_RELATED_OFF, 4, &val);
	val |= EQUALIZATION_DISABLE;
	exynos_pcie_wr_own_conf(pp, PCIE_GEN3_RELATED_OFF, 4, val);
 ...
 * @prdcode
 * @unit_name{pcie_rc_internal}
 * @purpose "Set outbound for cfg"
 * @logic "Set outbound for cfg"
 * @params
 * @param{in/out, pp, struct ::pcie_port *, not NULL}
		...
		u32 type)
{
	...
	exynos_atu_write(exynos_pcie, pp->cfg0_base,
			PCIE_ATU_LOWER_BASE_OUTBOUND0);
	exynos_atu_write(exynos_pcie, (pp->cfg0_base >> 32),
			PCIE_ATU_UPPER_BASE_OUTBOUND0);
	exynos_atu_write(exynos_pcie, pp->cfg0_base + pp->cfg0_size - 1,
			PCIE_ATU_LIMIT_OUTBOUND0);
	exynos_atu_write(exynos_pcie, busdev, PCIE_ATU_LOWER_TARGET_OUTBOUND0);
	exynos_atu_write(exynos_pcie, 0, PCIE_ATU_UPPER_TARGET_OUTBOUND0);
	exynos_atu_write(exynos_pcie, type, PCIE_ATU_CR1_OUTBOUND0);
	exynos_atu_write(exynos_pcie, PCIE_ATU_ENABLE, PCIE_ATU_CR2_OUTBOUND0);
...
static void exynos_pcie_prog_viewport_mem_outbound(struct pcie_port *pp)
{
	...
	exynos_atu_write(exynos_pcie,
				PCIE_ATU_TYPE_MEM, PCIE_ATU_CR1_OUTBOUND1);
	exynos_atu_write(exynos_pcie, exynos_pcie->mem_base,
				PCIE_ATU_LOWER_BASE_OUTBOUND1);
	exynos_atu_write(exynos_pcie, (exynos_pcie->mem_base >> 32),
			PCIE_ATU_UPPER_BASE_OUTBOUND1);
	exynos_atu_write(exynos_pcie, exynos_pcie->mem_base + exynos_pcie->mem_size - 1,
			PCIE_ATU_LIMIT_OUTBOUND1);
	exynos_atu_write(exynos_pcie, exynos_pcie->mem_bus_addr,
			PCIE_ATU_LOWER_TARGET_OUTBOUND1);
	exynos_atu_write(exynos_pcie, upper_32_bits(exynos_pcie->mem_bus_addr),
			PCIE_ATU_UPPER_TARGET_OUTBOUND1);
	exynos_atu_write(exynos_pcie, PCIE_ATU_ENABLE,
				PCIE_ATU_CR2_OUTBOUND1);
}
	...
	int vf_num, u64 base_addr, u64 target_addr, u32 size, u64 vf_bar_size)
{
	...
	if (vf_num <= exynos_pcie->num_vfs) {
		...
		exynos_atu_write(exynos_pcie, PCIE_ATU_TYPE_MEM,
			PCIE_ATU_CR1_OUTBOUND0 + offset);
		exynos_atu_write(exynos_pcie, lower_32_bits(vf_base_addr),
			PCIE_ATU_LOWER_BASE_OUTBOUND0 + offset);
		exynos_atu_write(exynos_pcie, upper_32_bits(vf_base_addr),
			PCIE_ATU_UPPER_BASE_OUTBOUND0 + offset);
		exynos_atu_write(exynos_pcie, (vf_base_addr + size - 1),
			PCIE_ATU_LIMIT_OUTBOUND0 + offset);
		exynos_atu_write(exynos_pcie, lower_32_bits(vf_target_addr),
			PCIE_ATU_LOWER_TARGET_OUTBOUND0 + offset);
		exynos_atu_write(exynos_pcie, upper_32_bits(vf_target_addr),
			PCIE_ATU_UPPER_TARGET_OUTBOUND0 + offset);
		exynos_atu_write(exynos_pcie, PCIE_ATU_ENABLE,
			PCIE_ATU_CR2_OUTBOUND0 + offset);

		dev_info(pci->dev, "[%s] vf_num: %d base_addr: 0x%llx target_addr: 0x%llx offset: 0x%x\n",
...
retry:
	...
	linkA = exynos_phy_pcs_read(exynos_pcie, 0x988);
	linkB = exynos_phy_pcs_read(exynos_pcie, 0x188);
...
void exynos_pcie_hardaccess_work(struct work_struct *work)
{
	struct exynos_pcie *exynos_pcie =
	...
	/* DBI access */
	if (chk_hard_access(exynos_pcie, exynos_pcie->rc_dbi_base)) {
		dev_info(pci->dev, "DBI hared access test FAIL!\n");
		return;
	}

	/* EP configuration access */
	if (chk_hard_access(exynos_pcie, pp->va_cfg0_base)) {
	...
	if (chk_hard_access(exynos_pcie, ep_mem_region)) {
...
static void exynos_pcie_assert_phy_reset(struct pcie_port *pp)
{
	...
	if (exynos_pcie->phy_ops.phy_config != NULL)
		exynos_pcie->phy_ops.phy_config(pp);
 ...
 * @noret
 */
static void exynos_handle_msi_irq(struct pcie_port *pp)
{
	u32 val;
	...
	for (i = 0; i < MAX_MSI_SET; i++) {
		exynos_pcie_rd_own_conf(pp, PCIE_MSI_CTRL_INTR0_STATUS +
			i * PCIE_MSI_CTRL_INT_OFFSET, 4, &val);
		exynos_pcie_wr_own_conf(pp, PCIE_MSI_CTRL_INTR0_STATUS +
			i * PCIE_MSI_CTRL_INT_OFFSET, 4, val);
...
static void exynos_pcie_msi_init(struct pcie_port *pp)
{
	...
	exynos_pcie_wr_own_conf(pp, PCIE_MSI_CTRL_INTR0_ENABLE, 4, val);
}
#endif

static void __iomem *exynos_pcie_own_conf_map_bus(struct pci_bus *bus,
	unsigned int devfn, int where)
{
	return dw_pcie_own_conf_map_bus(bus, devfn, where);
}

static int exynos_generic_own_config_read(struct pci_bus *bus, unsigned int devfn,
	int where, int size, u32 *val)
{
	...
	if (is_linked == 0) {
		...
		exynos_pcie_phy_clock_enable(pp, PCIE_ENABLE_CLOCK);
	}
...
static int exynos_generic_own_config_write(struct pci_bus *bus, unsigned int devfn,
	int where, int size, u32 val)
{
	struct pcie_port *pp = bus->sysdata;
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
...
static void __iomem *exynos_pcie_other_conf_map_bus(struct pci_bus *bus,
	unsigned int devfn, int where)
{
	struct pcie_port *pp = bus->sysdata;
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
	...
	exynos_pcie_prog_viewport_cfg(pp, busdev, type);

	return pp->va_cfg0_base + where;
}

static int exynos_generic_other_config_read(struct pci_bus *bus, unsigned int devfn,
		int where, int size, u32 *val)
{
	struct pcie_port *pp = bus->sysdata;
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
...
static int exynos_generic_other_config_write(struct pci_bus *bus, unsigned int devfn,
		int where, int size, u32 val)
{
	struct pcie_port *pp = bus->sysdata;
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
...
static struct pci_ops exynos_own_pcie_ops = {
	.map_bus = exynos_pcie_own_conf_map_bus,
	.read = exynos_generic_own_config_read,
	.write = exynos_generic_own_config_write,
};

static struct pci_ops exynos_child_pcie_ops = {
	.map_bus = exynos_pcie_other_conf_map_bus,
	.read = exynos_generic_other_config_read,
	.write = exynos_generic_other_config_write,
};
		...
		struct pci_bus *bus, u32 devfn, int where, int size, u32 val)
{
	...
	ret = exynos_pcie_write_other(exynos_pcie,
		va_cfg_base + where, size, val);
 ...
 * @noret
 */
static void exynos_pcie_msi_irq_ack(struct irq_data *data)
{
 ...
 * @noret
 */
static void exynos_pcie_compose_msi_msg(struct irq_data *data,
							struct msi_msg *msg)
{
	struct pcie_port *pp = irq_data_get_irq_chip_data(data);
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
 ...
 * @noret
 */
static void exynos_pcie_msi_mask(struct irq_data *data)
{
	struct pcie_port *pp = irq_data_get_irq_chip_data(data);
#if IS_ENABLED(CONFIG_PCI_EXYNOS_IOV)
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
	...
	exynos_pcie_rd_own_conf(pp, PCIE_MSI_CTRL_INTR0_ENABLE
		+ res * PCIE_MSI_CTRL_INT_OFFSET, 4, &val);
	val &= ~(1 << irq);
	exynos_pcie_wr_own_conf(pp, PCIE_MSI_CTRL_INTR0_ENABLE
		+ res * PCIE_MSI_CTRL_INT_OFFSET, 4, val);
 ...
 * @noret
 */
static void exynos_pcie_msi_unmask(struct irq_data *data)
{
	struct pcie_port *pp = irq_data_get_irq_chip_data(data);
#if IS_ENABLED(CONFIG_PCI_EXYNOS_IOV)
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
...
static struct irq_chip exynos_pcie_msi_irq_chip = {
	.name = "EXYNOS-PCI-MSI",
	.irq_ack = exynos_pcie_msi_irq_ack,
	.irq_compose_msi_msg = exynos_pcie_compose_msi_msg,
	.irq_set_affinity = exynos_pcie_msi_set_affinity,
	.irq_mask = exynos_pcie_msi_mask,
	.irq_unmask = exynos_pcie_msi_unmask,
};
 ...
 * @retval{-, int, 0, 0, not 0}
 */
static int exynos_pcie_irq_domain_alloc(struct irq_domain *domain,
 ...
 * @noret
 */
static void exynos_pcie_irq_domain_free(struct irq_domain *domain,
				    unsigned int virq, unsigned int nr_irqs)
{
...
static const struct irq_domain_ops exynos_pcie_msi_domain_ops = {
	.alloc	= exynos_pcie_irq_domain_alloc,
	.free	= exynos_pcie_irq_domain_free,
};
 ...
 * @noret
 */
static void exynos_dw_msi_ack_irq(struct irq_data *d)
{
 ...
 * @noret
 */
static void exynos_dw_msi_mask_irq(struct irq_data *d)
{
	pci_msi_mask_irq(d);
 ...
 * @noret
 */
static void exynos_dw_msi_unmask_irq(struct irq_data *d)
{
...
static struct irq_chip exynos_dw_pcie_msi_irq_chip = {
	.name = "PCI-MSI",
	.irq_ack = exynos_dw_msi_ack_irq,
	.irq_mask = exynos_dw_msi_mask_irq,
	.irq_unmask = exynos_dw_msi_unmask_irq,
};
 ...
 * @retval{-, int, 0, 0, not 0}
 */
static int exynos_pcie_msi_host_init(struct pcie_port *pp)
{
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
...
static struct dw_pcie_host_ops exynos_pcie_host_ops = {
	...
	.msi_host_init = exynos_pcie_msi_host_init,
};
				...
				struct platform_device *pdev)
{
	...
	if (IS_ENABLED(CONFIG_PCI_MSI)) {
		...
		ret = devm_request_irq(&pdev->dev,
			exynos_pcie->msi_irq[0], exynos_pcie_msi_irq_handler,
 ...
 * @noret
 */
static void enable_cache_coherency(struct pcie_port *pp, int enable)
{
 ...
 */
static int exynos_pcie_parse_fixed_dt(struct device_node *np,
				struct pcie_port *pp)
{
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
	struct exynos_pcie *exynos_pcie = to_exynos_pcie(pci);
 ...
 * @retval{-, int, 0, int, Not 0}
 */
static int exynos_pcie_parse_configurable_dt(struct device_node *np,
							struct pcie_port *pp)
{
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
	struct exynos_pcie *exynos_pcie = to_exynos_pcie(pci);
 ...
 * @retval{ret, int, 0, not NULL, not 0}
 */
static int exynos_pcie_pinconfig(struct pcie_port *pp,
 ...
 * @retval{ret, int, 0, not NULL, not 0}
 */
static int exynos_pcie_resource_mapping(struct pcie_port *pp,
						struct platform_device *pdev)
{
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
	struct exynos_pcie *exynos_pcie = to_exynos_pcie(pci);
	...
	temp_rsc = platform_get_resource_byname(pdev, IORESOURCE_MEM,
							"elbi_cmn");
	exynos_pcie->elbi_cmn_base = ioremap(temp_rsc->start,
			resource_size(temp_rsc));
	if (IS_ERR(exynos_pcie->elbi_cmn_base))
		return PTR_ERR(exynos_pcie->elbi_cmn_base);
	...
	exynos_pcie->phy_pcs_base = ioremap(temp_rsc->start,
			resource_size(temp_rsc));

	if (IS_ERR(exynos_pcie->phy_pcs_base))
		return PTR_ERR(exynos_pcie->phy_pcs_base);
	...
	exynos_pcie->atu_base = devm_ioremap_resource(&pdev->dev, temp_rsc);
	if (IS_ERR(exynos_pcie->atu_base))
		return PTR_ERR(exynos_pcie->atu_base);
...
static int exynos_pcie_probe(struct platform_device *pdev)
{
	...
	dev_info(pci->dev, "%s: start probe\n", __func__);
	...
	ret = exynos_pcie_parse_fixed_dt(pdev->dev.of_node, pp);
	if (ret)
		goto probe_fail;

	ret = exynos_pcie_parse_configurable_dt(pdev->dev.of_node, pp);
	if (ret)
		goto probe_fail;

	ret = exynos_pcie_pinconfig(pp, pdev, np);
	if (ret)
		goto probe_fail;

	ret = exynos_pcie_resource_mapping(pp, pdev);
	if (ret)
		goto probe_fail;
	...
	INIT_DELAYED_WORK(&exynos_pcie->hardaccess_work,
				exynos_pcie_hardaccess_work);

	platform_set_drvdata(pdev, exynos_pcie);

#ifdef CONFIG_S2MPU
	if (exynos_pcie->s2mpu_base) {
		if (hyp_call_s2mpu_protection(exynos_pcie->s2mpu_base) != 0)
			dev_info(pci->dev, "%s Fail s2mpu=%08x\n",
				 __func__, exynos_pcie->s2mpu_base);
		else
			dev_info(pci->dev, "%s Success s2mpu=%08x\n",
				__func__, exynos_pcie->s2mpu_base);
	} else {
		dev_info(pci->dev, "%s s2mpu is disabled [s2mpu=%08x]\n",
			__func__, exynos_pcie->s2mpu_base);
	}
#endif
...
int exynos_pcie_poweron(int ch_num)
{
	...
	if (exynos_pcie->state == STATE_LINK_DOWN) {
		...
		/* Enalbe PHY */
		dev_info(pci->dev, "[%s] start pmu bypass\n", __func__);
		regmap_update_bits(exynos_pcie->pmureg,
				exynos_pcie->pmu_phy_offset,
				PCIE_PHY_CONTROL_MASK, 1);
		...
		/* setup ATU for cfg/mem outbound */
		busdev = PCIE_ATU_BUS(exynos_pcie->pci_dev->subordinate->number) |
			PCIE_ATU_DEV(0) | PCIE_ATU_FUNC(0);

		if (pci_is_root_bus(exynos_pcie->pci_dev->bus))
			type = PCIE_ATU_TYPE_CFG0;
		else
			type = PCIE_ATU_TYPE_CFG1;
		...
		dev_info(pci->dev, "[%s]subordinate->number:%d, busdev:%x, type:%d\n",
			__func__, exynos_pcie->pci_dev->subordinate->number, busdev, type);
...
#if IS_ENABLED(CONFIG_PCI_EXYNOS_IOV)
		/* Update PMEM state LINK DOWN */
		if (exynos_pcie->sriov_supported == 1)
			exynos_vf_request_vlink_send(PF_LINKDOWN);
#endif
