...
static inline bool exynos_vpci_is_server(vpci_link_t *);
static int exynos_vpci_link_init(struct exynos_pcie_vf *pcie_vf,
	NkDevVlink *vlink, NkPhAddr plink, bool server);
static int exynos_vpci_init(struct exynos_pcie_vf *pcie_vf);
static void exynos_vpci_pmem_show(struct exynos_pcie_vf *pcie_vf);
static void exynos_pcie_vf_msi_clear_irq(struct exynos_pcie_vf *pcie_vf);
static void exynos_pcie_vf_msi_set_irq(struct exynos_pcie_vf *pcie_vf);
 ...
 * @noret
 */
static inline void exynos_vpci_lock_init(vpci_link_t *link)
{
 ...
 * @noret
 */
static inline void exynos_vpci_lock(vpci_link_t *link)
{
	unsigned long flags;
 ...
 * @noret
 */
static inline void exynos_vpci_unlock(vpci_link_t *link)
{
 ...
 * @noret
 */
static void exynos_vpci_xirq_post(vpci_link_t *link)
{
	exynos_vpci_lock(link);
 ...
 * @noret
 */
static void exynos_vpci_sysconf_post(vpci_link_t *link)
{
 ...
 * @noret
 */
static void exynos_vpci_pmem_reset(vpci_link_t *link)
{
	(void)link;
}
 ...
 * @noret
 */
static void exynos_vpci_pmem_init(vpci_link_t *link)
{
 ...
 */
bool exynos_vpci_link_is_online(vpci_link_t *link)
{
 ...
 * @retval{-, bool, 0, 0/1, not 0/1}
 */
static bool exynos_vpci_handshake(vpci_link_t *link)
{
	volatile int *my_state;
	int peer_state;
 ...
 * @noret
 */
static void exynos_vpci_xirq_handler(void *cookie, NkXIrq xirq)
{
	struct exynos_pcie_vf *pcie_vf = cookie;
	struct device *dev = pcie_vf->pci->dev;

	exynos_vpci_lock(pcie_vf->vpci[pcie_vf->vlink_num]);
	...
	if (pcie_vf->vpci[0]->server) {
		...
		exynos_vpci_pmem_show(pcie_vf);
	}
	exynos_vpci_unlock(pcie_vf->vpci[pcie_vf->vlink_num]);
}
 ...
 */
static int exynos_vpci_link_ready(vpci_link_t *link)
{
	int ret = 0;

	exynos_vpci_lock(link);
	ret = exynos_vpci_handshake(link);
	exynos_vpci_unlock(link);
	return ret;
}
 ...
 * @noret
 */
static void exynos_vpci_sysconf_handler(void *cookie, NkXIrq xirq)
{
	struct exynos_pcie_vf *pcie_vf = cookie;
	int i;
	...
	if (pcie_vf->vpci[0]->running) {
		if (*pcie_vf->vpci[0]->peer.state != NK_DEV_VLINK_ON ||
			*pcie_vf->vpci[0]->my.state != NK_DEV_VLINK_ON)
			exynos_vpci_link_ready(pcie_vf->vpci[0]);
	}
 ...
 * @retval{-, bool, 0, 0/1, not 0/1}
 */
static inline bool exynos_vpci_is_server(vpci_link_t *link)
{
	return link->server;
}
 ...
 * @retval{-, bool, 0, 0/1, not 0/1}
 */
static inline bool exynos_vpci_is_client(vpci_link_t *link)
{
	return !link->server;
}
 ...
 */
static int exynos_vpci_my_xirq_id(struct exynos_pcie_vf *pcie_vf,
					vpci_link_t *link)
{
	int id = 0;
	int ret = 0;
	...
	ret = (exynos_vpci_is_server(link) ? id : 0);
	return ret;
}
 ...
 * @retval{ret, int, 0, 0~4, not 0~4}
 */
static int exynos_vpci_peer_xirq_id(struct exynos_pcie_vf *pcie_vf,
					vpci_link_t *link)
{
	int id = 0;
	int ret = 0;
	...
	ret = (exynos_vpci_is_server(link) ? 0 : id);
	return  ret;
}
 ...
 * @retval{-, bool, 0, 0/1, not 0/1}
 */
static bool exynos_vpci_pxirq_alloc(struct exynos_pcie_vf *pcie_vf,
					vpci_link_t *link)
{
	NkPhAddr plink = link->plink;
	int mid = exynos_vpci_my_xirq_id(pcie_vf, link);
	int pid = exynos_vpci_peer_xirq_id(pcie_vf, link);
 ...
 * @noparam
 * @retval{size, vpci_size_t, 1024, 1024, not 1024}
 */
static vpci_size_t pmem_info_size(void)
{
	vpci_size_t size = VF_PMEM_DEF_SIZE;
	return size;
}
 ...
 */
static vpci_size_t exynos_vpci_pmem_size(void)
{
	vpci_size_t msize = pmem_info_size();
	vpci_size_t psize = pmem_info_size();
 ...
 * @retval{-, bool, 0, 0/1, not 0/1}
 */
static bool exynos_vpci_pmem_alloc(vpci_link_t *link)
{
	vpci_size_t size = exynos_vpci_pmem_size();
 ...
 * @retval{-, bool, 0, 0/1, not 0/1}
 */
static bool exynos_vpci_shared_mem_alloc(vpci_link_t *link)
{
	...
	NkDevVlink *vlink, NkPhAddr plink, bool server)
{
	...
	vpci_link = (vpci_link_t *)kzalloc(sizeof(vpci_link_t), GFP_KERNEL);
	pcie_vf->vpci[pcie_vf->vlink_cnt] = vpci_link;
	if (!vpci_link) {
		dev_err(dev, "vpci_link ERROR");
		return -ENOMEM;
	}
	...
	if (exynos_vpci_is_server(vpci_link)) {
	...
	if (!exynos_vpci_pmem_alloc(vpci_link)) {
		dev_err(dev, "exynos_vpci_pmem_alloc ERROR\n");
		return -EINVAL;
	}


	pcie_vf->scid = nkops.nk_xirq_attach(NK_XIRQ_SYSCONF,
			exynos_vpci_sysconf_handler, pcie_vf);
	if (!pcie_vf->scid) {
		mutex_unlock(&_mutex);
	...
	if (!exynos_vpci_pxirq_alloc(pcie_vf, vpci_link)) {
		dev_err(dev, "exynos_vpci_pxirq_alloc ERROR\n");
		return -EINVAL;
	}

	if (!exynos_vpci_shared_mem_alloc(vpci_link)) {
		dev_err(dev, "exynos_vpci_shared_mem_alloc ERROR\n");
		return -EINVAL;
	}

	vpci_link->xid = nkops.nk_xirq_attach(vpci_link->my.xirq,
			exynos_vpci_xirq_handler, pcie_vf);
...
static void exynos_vpci_pmem_show(struct exynos_pcie_vf *pcie_vf)
{
	...
	if (pcie_vf->vpci[0]->status == PF_LINKUP) {
		exynos_pcie_vf_msi_clear_irq(pcie_vf);
		exynos_pcie_vf_msi_set_irq(pcie_vf);
	}
}
 ...
 * @noret
 */
static void exynos_vpci_pmem_set(struct exynos_pcie_vf *pcie_vf, int status)
{
	struct device *dev = pcie_vf->pci->dev;
	int i;
	...
	if (exynos_vpci_link_is_online(pcie_vf->vpci[0])) {
	...
	if (pcie_vf->backend) {
		for (i = 1; i < pcie_vf->vlink_cnt; i++) {
			...
			if (exynos_vpci_link_is_online(pcie_vf->vpci[i])) {
				memcpy(
				(int *)pcie_vf->vpci[i]->shared.pmem.vaddr,
						&status, sizeof(int));
			} else {
 ...
 * @noret
 */
static void exynos_vpci_intr_set(struct exynos_pcie_vf *pcie_vf)
{
	struct device *dev = pcie_vf->pci->dev;
	int i = 0;
	...
	if (exynos_vpci_link_is_online(pcie_vf->vpci[0]))
		exynos_vpci_xirq_post(pcie_vf->vpci[0]);
	else
		dev_info(dev, "V Link 0 is not online\n");
	...
	if (pcie_vf->vpci[0]->server) {
		for (i = 1; i < pcie_vf->vlink_cnt; i++) {
			if (exynos_vpci_link_is_online(pcie_vf->vpci[i]))
				exynos_vpci_xirq_post(pcie_vf->vpci[i]);
			else
				dev_info(dev, "V Link %d is not online\n", i);
		}
	}
...
void exynos_vf_request_vlink_send(int value)
{
	...
	if (!exynos_vpci_link_is_online(pcie_vf->vpci[0])) {
		...
		while (!exynos_vpci_link_is_online(pcie_vf->vpci[0]) &&
			timeout < 20000) {
			udelay(10);
			timeout++;
		}
	...
	if (pcie_vf->backend) {
		for (i = 1; i < pcie_vf->vlink_cnt; i++) {
			if (!exynos_vpci_link_is_online(pcie_vf->vpci[i])) {
				...
				exynos_vpci_link_ready(pcie_vf->vpci[i]);

				timeout = 0;

				while (!exynos_vpci_link_is_online
	...
	exynos_vpci_pmem_set(&g_pcie_vf, value);
	exynos_vpci_intr_set(&g_pcie_vf);
}
EXPORT_SYMBOL(exynos_vf_request_vlink_send);
...
static int exynos_vpci_init(struct exynos_pcie_vf *pcie_vf)
{
	...
	while ((plink = nkops.nk_vlink_lookup("vpci", plink)) != 0) {
		...
		if (vlink->s_id == myid) {
			...
			res = exynos_vpci_link_init(pcie_vf, vlink,
							plink, true);
			if (res != 0) {
				mutex_unlock(&_mutex);
 ...
 * @retval{-, int, 0, -ERANGE~0, <0}
 */
static int exynos_pcie_vf_parse_dt(struct exynos_pcie_vf *pcie_vf,
 ...
 */
static int exynos_pcie_vf_get_resource(struct exynos_pcie_vf *pcie_vf)
{
 ...
 */
static irqreturn_t exynos_pcie_vf_irq_handler(int irq, void *arg)
{
 ...
 */
static ssize_t show_pcie_vf(struct device *dev,
 ...
 */
static ssize_t store_pcie_vf(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	...
	case 5:
		if (pcie_vf->vf_num == 0)
			exynos_vpci_pmem_set(pcie_vf, 1);
		else
			exynos_vpci_pmem_set(pcie_vf, 2);

		break;

	case 6:
		exynos_vpci_intr_set(pcie_vf);
		break;
...
static DEVICE_ATTR(vf_sysfs, S_IWUSR | S_IWGRP | S_IRUSR | S_IRGRP,
			show_pcie_vf, store_pcie_vf);
 ...
 */
static int exynos_pcie_vf_rmem_init(struct reserved_mem *rmem)
{
...
RESERVEDMEM_OF_DECLARE(pcie_vf_rmem, "exynosauto9,pcie-vf-rmem",
				exynos_pcie_vf_rmem_init);
#endif
 ...
 */
static int exynos_pcie_vf_probe(struct platform_device *pdev)
{
	...
	ret = exynos_pcie_vf_parse_dt(pcie_vf, pdev->dev.of_node);
	if (ret)
		goto probe_fail;

	ret = exynos_pcie_vf_get_resource(pcie_vf);
	if (ret) {
	...
	ret = request_irq(pcie_vf->irq, exynos_pcie_vf_irq_handler,
 ...
 * @retval{-, int, 0, 0, not 0}
 */
static int exynos_pcie_vf_remove(struct platform_device *pdev)
{
	dev_info(&pdev->dev, "Remove EXYNOS PCIe VF driver\n");
	return 0;
}

static const struct of_device_id exynos_pcie_vf_of_match[] = {
	{ .compatible = "samsung,exynos-pcie-vf", },
	{},
};
MODULE_DEVICE_TABLE(of, exynos_pcie_vf_of_match);

static struct platform_driver exynos_pcie_vf_driver = {
	.remove		= exynos_pcie_vf_remove,
	.driver = {
		.name	= "exynos-pcie-vf",
		.owner	= THIS_MODULE,
		.of_match_table = exynos_pcie_vf_of_match,
	},
};
