 ...
 */
static int exynos_v920_pcie_ep_clk_enable(struct exynos_ep_pcie *pcie_ep, int enable)
{
...
void exynos_v920_pcie_ep_phyinit(struct exynos_ep_pcie *pcie_ep)
{
	...
	pcie_ep->phy_ops.phy_all_pwrdn = exynos_v920_pcie_ep_phy_all_pwrdn;
	pcie_ep->phy_ops.phy_all_pwrdn_clear =
					exynos_v920_pcie_ep_phy_all_pwrdn_clear;
	pcie_ep->phy_ops.phy_config = exynos_v920_pcie_ep_phy_config;
	pcie_ep->phy_ops.phy_other_set = exynos_v920_pcie_ep_phy_others_set;
}
...
static int exynos_v920_pcie_ep_get_resource(struct exynos_ep_pcie *pcie_ep)
{
	...
	/* To Enable Dummy Qch, Remove in EVT2 */
...
static void exynos_v920_pcie_setup_ep(struct exynos_ep_pcie *pcie_ep)
{
	...
	val = dw_pcie_readl_dbi(pci, 0x0);
	val = val & 0x0000FFFF;
	dw_pcie_writel_dbi(pci, 0x0, (0xa582)<<16|val);

	/* RC BAR disable */
	val = dw_pcie_readl_dbi(pci, 0x50);
	val &= ~((0x7)<<17);
	val |= 0x5 << 17;
	dev_info(&pdev->dev, "[%s] Set to MSI count 32 = 0x%x\n", __func__, val);
	dw_pcie_writel_dbi(pci, 0x50, val);
 ...
 * @noret
 */
static void exynos_v920_pcie_ep_atomic_op(struct exynos_ep_pcie *pcie_ep, int enable)
{
	struct platform_device *pdev = pcie_ep->pdev;
	u32 val = 0;
 ...
 * @noret
 */
static void exynos_v920_pcie_ep_set_ltssm(struct exynos_ep_pcie *pcie_ep, int enable)
{
	struct platform_device *pdev = pcie_ep->pdev;
	u32 val;
...
static int exynos_v920_pcie_ep_setup_interrupt(struct exynos_ep_pcie *pcie_ep)
{
	...
	writel(PCIE_EP_INTADMASK, pcie_ep->elbi_base + PCIE_INT_STS);

	writel(PCIE_EP_IRQMASK, pcie_ep->elbi_base + PCIE_MISC_INT_EN);

	writel(PCIE_EP_IRQMASK, pcie_ep->elbi_base + PCIE_RX_MSG_INT_EN);
...
static void exynos_v920_ep_ready_establish_link(struct exynos_ep_pcie *pcie_ep)
{
	...
	if (gpio_get_value(pcie_ep->perst_gpio) ||
		(readl(pcie_ep->sysreg_base + pcie_ep->sysreg_offset) & 0xf0) >> 4 == 0x1) {
		...
		exynos_v920_pcie_ep_setup_interrupt(pcie_ep);

		dev_info(&pdev->dev, "[%s] Ready to establish link \n", __func__);
	} else
		dev_info(&pdev->dev, "[%s] Ready to establish link fail, PERST not high\n", __func__);
}
 ...
 */
static unsigned int exynos_v920_pcie_ep_check_linksts(struct exynos_ep_pcie *pcie_ep,
							unsigned int prests)
{
	struct platform_device *pdev = pcie_ep->pdev;
	unsigned int linksts =
		readl(pcie_ep->elbi_base + 0x304) & 0xff;
...
static void exynos_v920_pcie_ep_bar_setting(struct exynos_ep_pcie *pcie_ep)
{
	...
	exynos_v920_pcie_ep_set_bar_inbound(pcie_ep, 0, pcie_ep->ch_num);
}
	...
	if (pcie_ep->ch_num % 2 == 0) {
		...
		if (other_lane_status == -1) {
			...
			pcie_ep->ep_phyinfo->linkB_status =
					readl(pcie_ep->elbi_base_other) & 0x1f;
		} else {
			/* Other link is EP */
 ...
 */
static unsigned int exynos_v920_pcie_ep_link_start(struct exynos_ep_pcie *pcie_ep)
{
	struct platform_device *pdev = pcie_ep->pdev;
...
start_linkup:
	if (pcie_ep->linkup_ready == 1) {
		/* 1. phy initialize */
		pcie_ep->phy_ops.phy_config(pcie_ep->ep_phyinfo, &ret);
		if (ret < 0) {
			dev_info(&pdev->dev,
		...
		/* 5. pcie atomic op */
		exynos_v920_pcie_ep_atomic_op(pcie_ep, 1);
		/* 6. pcie_setiATU */
		/* 7. enable LTSSM */
		exynos_v920_pcie_ep_set_ltssm(pcie_ep, 1);
		...
		if ((readl(pcie_ep->sysreg_base + pcie_ep->sysreg_offset) & 0xf0) >> 4 == 0x1) {
 ...
 * @retval{-, int, 0, 0, not 0}
 */
static int exynos_v920_pcie_ep_clear_bar_force
 ...
 * @noret
 */
void exynos_v920_pcie_ep_irq2_evt(struct exynos_ep_pcie *pcie_ep,
						u32 value, u32 mask)
{
	struct platform_device *pdev = pcie_ep->pdev;
	u32 val = 0x0;
...
void exynos_v920_pcie_ep_irq1_handler(struct work_struct *work)
{
	...
	if (pcie_ep->ch_num == 0)
		other_ch = 1;
	else if (pcie_ep->ch_num == 1)
		other_ch = 0;
	else if (pcie_ep->ch_num == 2)
		other_ch = 3;
	else if (pcie_ep->ch_num == 3)
		other_ch = 2;

	if (pcie_ep->use_bifurcation) {
		while (cnt--) {
			if (g_pcie_ep_v920[other_ch].state == STATE_LINK_UP ||
				g_pcie_ep_v920[other_ch].state == STATE_LINK_DOWN)
				break;
			udelay(10);
		}
	}
...
static irqreturn_t exynos_v920_pcie_ep_irq_handler(int irq, void *arg)
{
	...
	pcie_ep->irq1_status = readl(elbi_base + PCIE_MISC_INT_STS);
	writel(pcie_ep->irq1_status, elbi_base + PCIE_MISC_INT_STS);

	/* handle IRQ2 interrupt (PM_in L2 int) */
	pcie_ep->irq2_status = readl(elbi_base + PCIE_PM_STS);
	writel(pcie_ep->irq2_status, elbi_base + PCIE_PM_STS);
...
int exynos_v920_pcie_ep_msi(struct exynos_ep_pcie *pcie_ep, int msi_data)
{
	...
	dev_info(dev, "write + allignd MSI: %x\n", msi_data);

	aligned_offset = lmsi_pci_addr & (epc->mem->window.page_size - 1);
	writel(msi_data, dst_addr + aligned_offset);
 ...
 * @retval{ret, int, 0, -ERANGE~0, <0}
 */
int exynos_v920_pcie_ep_write(struct exynos_ep_pcie *pcie_ep, u64 dst_pci_addr,
	int dma_num)
{
	struct platform_device *pdev = pcie_ep->pdev;
	struct device *dev = &pdev->dev;
 ...
 * @retval{ret, int, 0, -ERANGE~0, <0}
 */
int exynos_v920_pcie_ep_read(struct exynos_ep_pcie *pcie_ep, u64 src_pci_addr,
...
void exynos_v920_pcie_ep_work_cmd(struct work_struct *work)
{
	...
	case WORK_MEM_WR:
		...
		if (exynos_v920_pcie_ep_write(pcie_ep,
	...
	case WORK_MEM_RD:
		...
		if (exynos_v920_pcie_ep_read(pcie_ep, pci_addr, pcie_ep->size,
			pcie_ep->dma_num) != 0)
	...
	case WORK_DOORBELL:
		/* Received Test Read Request */
		pci_addr = RD_DMA_TEST_ADDR;
		if (exynos_v920_pcie_ep_read(pcie_ep,
 ...
 */
static ssize_t exynos_v920_pcie_ep_show_dbg(struct device *dev,
 ...
 */
static ssize_t exynos_v920_pcie_ep_store_dbg(struct device *dev,
		struct device_attribute *attr,
...
static DEVICE_ATTR(ep_dbg, S_IWUSR | S_IWGRP | S_IRUSR | S_IRGRP,
			exynos_v920_pcie_ep_show_dbg, exynos_v920_pcie_ep_store_dbg);
/**
 * @cnotice
 * @prdcode
 * @unit_name{pcie_ep_internal}
...
static int exynos_v920_pcie_ep_probe(struct platform_device *pdev)
{
	...
	if (pcie_ep->ch_num % 2 == 0)
		g_pcie_ep_v920[ch_num + 1].lane_status = -1;
	else
		g_pcie_ep_v920[ch_num - 1].lane_status = -1;
	...
	if (pcie_ep->ch_num == 0)
		pcie_ep->sysreg_offset = FSYS0_PCIE_GEN3_4LA_PHY_CTRL;
	else if (pcie_ep->ch_num == 1)
		pcie_ep->sysreg_offset = FSYS0_PCIE_GEN3_4LB_PHY_CTRL;
	else if (pcie_ep->ch_num == 2)
		pcie_ep->sysreg_offset = FSYS0_PCIE_GEN3_2LA_PHY_CTRL;
	else if (pcie_ep->ch_num == 3)
		pcie_ep->sysreg_offset = FSYS0_PCIE_GEN3_2LB_PHY_CTRL;
...
static const struct of_device_id exynos_pcie_ep_of_match[] = {
	{ .compatible = "samsung,exynos-v920-pcie-ep", },
	{},
};
