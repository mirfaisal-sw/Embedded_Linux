                          ...
                          char *pfw_path, char *pnv_path)
{
	...
	if (concate_revision(bus, bus->fw_path, bus->nv_path) != 0) {
		...
		pr_alert("MIR: In function - %s, line - %d, ret - %d\n", __func__, __LINE__, ret);
		return BCME_BADARG;
	...
	pr_alert("MIR: Going to dump resource in func - %s, line - %d\n", __func__, __LINE__);
...
dhdpcie_download_code_file(struct dhd_bus *bus, char *pfw_path)
{
	...
	if (dhd_bus_get_fw_mode(bus->dhd) == DHD_FLAG_MFG_MODE) {
		...
		pr_alert("MIR: In function - %s, dhd_tcm_test_enable - %d\n", __func__, dhd_tcm_test_enable);
	}
