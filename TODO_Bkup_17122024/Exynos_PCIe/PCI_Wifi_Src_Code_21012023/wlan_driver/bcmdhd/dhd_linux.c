...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	if  (dhd->pub.busstate == DHD_BUS_DOWN && dhd_update_fw_nv_path(dhd)) {
		...
		if (ret < 0) {
			...
			DHD_PERIM_UNLOCK(dhdp);
			pr_alert("MIR: func - %s failed to load fw\n", __func__);
			return ret;
		}
...
#ifdef PCIE_FULL_DONGLE
	{
		...
		if ((ret = dhd_flow_rings_init(&dhd->pub, max_h2d_rings)) != BCME_OK) {
			...
			pr_alert("MIR: func - %s line - %d, ret val - %d\n", __func__, __LINE__, ret);
			return ret;
		}
	}
#endif /* PCIE_FULL_DONGLE */
...
#ifdef DHD_SUPPORT_REQFW_FOR_FIRMWARE_DOWNLOADING
int
dhd_os_open_reqfw(dhd_pub_t *pub, const struct firmware **fw, char *filename)
{
	int ret = BCME_OK;

	ret = request_firmware(fw, filename, dhd_bus_to_dev(pub->bus));
...
dhd_os_open_image1(dhd_pub_t *pub, char *filename)
{
	...
	if (fw) {
		ret = dhd_os_open_reqfw(pub, &fw->fw, filename);

		pr_alert("MIR: In func - %s, fw filename - %s, line - %d, ret - %d\n", __func__, filename, __LINE__, ret);
		//dump_stack_lvl(KERN_ALERT);
		if (!ret) {
			fw->offset = 0;
		}
		else {
	...
	pr_alert("MIR: In func - %s, fw filename - %s, line - %d\n", __func__, filename, __LINE__);
	 ...
	 */
	 if (IS_ERR(fp)) {
		 fp = NULL;
		 pr_alert("MIR: In function - %s, line - %d\n", __func__, __LINE__);
		 //dump_stack_lvl(KERN_ALERT);
...
#endif /* defined(WL_DHD_XR_MASTER) || defined(WL_DHD_XR_CLIENT) */
static void
