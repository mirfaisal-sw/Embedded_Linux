...
int dhd_sync_with_dongle(dhd_pub_t *dhd)
{
	...
	pr_alert("MIR: MAC addr of WLAN chip - %x.%x.%x.%x.%x.%x\n", dhd->mac.octet[0], \
                        dhd->mac.octet[1], dhd->mac.octet[2], dhd->mac.octet[3], \
                        dhd->mac.octet[4], dhd->mac.octet[5]);
#endif /* OEM_ANDROID */
...
int dhd_prot_ioctl(dhd_pub_t *dhd, int ifidx, wl_ioctl_t * ioc, void * buf, int len)
{
	...
	if (action & WL_IOCTL_ACTION_SET) {
		...
		pr_alert("MIR: In fucntion - %s, line - %d, ret - %d\n", __func__, __LINE__, ret);
