		...
		if (wl_get_drv_status_all(cfg, FINDING_COMMON_CHANNEL)) {
			...
			if (cfg->parallel_scan_cnt == 0) {
				...
				if (DHD_XR_GET_SLAVE_WDEV(cfg) != NULL) {
					wdev = (struct wireless_dev *)DHD_XR_GET_SLAVE_WDEV(cfg);
				} else {
				...
				pub = dhd_get_pub(wdev->netdev);
				mutex_unlock(&cfg->scan_sync);
#if defined(WL_CFG80211_P2P_DEV_IF)
				err =  wl_cfg80211_scan_xr(cfg->pub, pub, wiphy, cfg->scan_request);
#else
				err =  wl_cfg80211_scan_xr(cfg->pub, pub, wiphy, wdev->netdev, cfg->scan_request);
#endif /* WL_CFG80211_P2P_DEV_IF */

				if (unlikely(err)) {
					WL_ERR((" 5G scan error (%d) informing 2G scan results\n", err));
					wl_inform_bss(cfg);
					wl_notify_escan_complete(cfg, ndev, false, false);
				}
				return err;
...
#ifdef WL_DHD_XR
		if (request->channels[i]->band != band)
			continue;
#endif /* WL_DHD_XR */
...
/*Trigger scan for both dhd1 and dhd2*/
		...
		WL_DBG(("n_channels_2g=%d,n_channels_5g=%d\n", n_channels_2g, n_channels_5g));
...
#ifdef WL_DHD_XR_MASTER
	wl_clr_drv_status(cfg, SCANNING, DHD_XR_GET_SLAVE_NDEV(cfg));
#endif /* WL_DHD_XR_MASTER */
