...
int wl_cfg80211_dhd_xr_detach() {
	struct bcm_cfg80211 *cfg = wl_cfg80211_get_bcmcfg();

	if (cfg->xr_slave_prim_wdev) {
...
EXPORT_SYMBOL(wl_cfg80211_dhd_xr_detach);
...
#endif /* WL_CFG80211_P2P_DEV_IF */
{
	...
	ret = dhd_send_xr_cmd(dest_pub, cmd, size, &cmd_wait->scan_wait, TRUE);
	...
	return xr_ctx->xr_cmd_reply_status.scan_status;
}

int wl_cfg80211_scan_xr_reply(dhd_pub_t *dest_pub, s32 status)
{
	xr_cmd_t *cmd = NULL;
	int size = sizeof(xr_cmd_t) + sizeof(xr_cmd_reply_scan_t);
	gfp_t flags = (in_atomic()) ? GFP_ATOMIC : GFP_KERNEL;
	xr_cmd_reply_scan_t *data = NULL;
	int ret = BCME_OK;
	...
	/*Create cmd*/
	cmd->cmd_id = XR_CMD_REPLY_SCAN;
	cmd->len = sizeof(xr_cmd_reply_scan_t);
	data = (xr_cmd_reply_scan_t *)&cmd->data[0];
...
int xr_cmd_scan_hndlr(dhd_pub_t *pub, xr_cmd_t *xr_cmd)
{
	...
	if (DHD_XR_GET_SLAVE_WDEV(cfg) != NULL) {
		cmd->ndev = DHD_XR_GET_SLAVE_NDEV(cfg);
		status = __wl_cfg80211_scan(cmd->wiphy, cmd->ndev, cmd->request, NULL);
	} else {
		DHD_ERROR(("cfg->xr_slave_prim_wdev is NULL \n"));
		ret = BCME_ERROR;
	}
#else
	status = __wl_cfg80211_scan(cmd->wiphy, cmd->ndev, cmd->request, NULL);
#endif /* WL_CFG80211_P2P_DEV_IF */
	if (dest_pub) {
		wl_cfg80211_scan_xr_reply(dest_pub, status);
	}

	return ret;
}

int xr_cmd_reply_scan_hndlr(dhd_pub_t *pub, xr_cmd_t *xr_cmd)
{
	int ret = BCME_OK;
	xr_cmd_reply_scan_t *reply = NULL;
	xr_ctx_t *xr_ctx = (xr_ctx_t *) DHD_GET_XR_CTX(pub);
	...
	reply = (xr_cmd_reply_scan_t *) &xr_cmd->data[0];
	xr_ctx->xr_cmd_reply_status.scan_status = reply->status;
	complete(&xr_ctx->xr_cmd_wait.scan_wait);
...
int xr_cmd_add_key_hndlr(dhd_pub_t *pub, xr_cmd_t *xr_cmd)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 2)) || defined(DHD_ANDROID_KERNEL5_15_SUPPORT)
	int link_id;
#endif
	uint8 xr_role = DHD_GET_XR_ROLE(pub);
	...
	status = wl_cfg80211_add_key(cmd->wiphy, cmd->dev,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 2)) || defined(DHD_ANDROID_KERNEL5_15_SUPPORT)
			link_id,
#endif
...
int xr_cmd_config_default_key_hndlr(dhd_pub_t *pub, xr_cmd_t *xr_cmd)
{
	...
	status = wl_cfg80211_config_default_key(cmd->wiphy, cmd->dev,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 2)) || defined(DHD_ANDROID_KERNEL5_15_SUPPORT)
			link_id,
#endif
...
int xr_cmd_del_key_hndlr(dhd_pub_t *pub, xr_cmd_t *xr_cmd)
{
	...
	status = wl_cfg80211_del_key(cmd->wiphy, cmd->dev, 
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 2)) || defined(DHD_ANDROID_KERNEL5_15_SUPPORT)
			link_id,
#endif
...
int xr_cmd_get_key_hndlr(dhd_pub_t *pub, xr_cmd_t *xr_cmd)
{
	...
	status = wl_cfg80211_get_key(cmd->wiphy, cmd->dev, 
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 2)) || defined(DHD_ANDROID_KERNEL5_15_SUPPORT)
		link_id,
#endif
		cmd->key_idx, cmd->pairwise, cmd->mac_addr, cmd->cookie, cmd->callback);
...
int xr_cmd_handler(dhd_pub_t *pub, xr_buf_t *xr_buf)
{
	...
	case XR_CMD_REPLY_SCAN:
	{
		ret = xr_cmd_reply_scan_hndlr(pub, xr_cmd);
		break;
	}
	...
	{
		ret = xr_cmd_del_virtual_iface_hndlr(pub, xr_cmd);
		break;
	}
