	...
	wl_iftype_t wl_iftype, const char *name, u8 *mac)
{
	...
	if (memcmp(name, "bcmx", 4) == 0) {
		...
		if (!cfg->xr_slave_prim_wdev) {
			...
			dest_pub = (dhd_pub_t *)dhd_get_pub(cfg->xr_slave_prim_wdev->netdev);
                                ...
                                const struct cfg80211_bitrate_mask *mask)
{
	...
	if (p2p_is_on(cfg) && (dev->ieee80211_ptr->iftype == NL80211_IFTYPE_P2P_GO)) {
              WL_ERR(("Setting HE rates for P2P GO is not supported\n"));
              return -ENOTSUPP;
        }
	...
	for (i = 0; i < NUM_NL80211_BANDS; i++) {
		...
		if (he) {
			...
			rspec |= (WL_RSPEC_HE_NSS_UNSPECIFIED << WL_RSPEC_HE_NSS_SHIFT) | mask->control[i].he_mcs[0];
