	...
	if (!bus->irq_registered) {
		...
		if (bus->d2h_intr_method == PCIE_MSI) {
			if (dhdpcie_enable_msi(pdev, 1, 1) < 0) {
				...
				pr_alert("MIR: dhdpcie_enable_msi() failed - %s\n", __func__);
...
int dhdpcie_get_resource(dhdpcie_info_t *dhdpcie_info)
{
	...
	do {
		...
		pr_alert("MIR: pa - %llu, va(dhdpcie_info->regs) - %p, function - %s\n", bar0_addr, dhdpcie_info->regs, __func__);
		...
		pr_alert("MIR: pa - %llu, va(dhdpcie_info->tcm) - %p, function - %s\n", bar1_addr, dhdpcie_info->tcm, __func__);
...
void dhdpcie_dump_resource(dhd_bus_t *bus)
{
	...
	pr_alert("MIR: In function - %s, Bar0 va - %p\n", __func__, pch->regs);
	/* BAR0 */
...
int dhdpcie_init(struct pci_dev *pdev)
{
	...
	do {
		...
		if (ret != BCME_OK) {
			...
			pr_alert("MIR: %s:dhdpcie_bus_attach() failed\n", __func__);
			break;
		}
