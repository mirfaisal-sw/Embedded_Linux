...
static uint8_t get_version_tx_buf[] = { 0x57, 0x44, 0x52, 0x00, 0x04, 0x00,
...
void sendCommandToTuner(uint8_t *packetBuf, uint8_t no_of_bytes);
...
void *write_function(void *arg)
{
	...
	{	
		...
		//sendCommandToTuner(get_config_tx_buf,16);
		sendCommandToTuner(get_version_tx_buf,16);
...
void *read_function(void *arg)
{
	...
	while(1)
	{
		...
		printf("No of successful read count - %d\n",gSuccessfulReadCount);
...
void write_m_times_read_n_times(int m_cnt, int n_cnt)
{
	...
	for(i = 0;i<m_cnt;i++)
	{
		...
		write(fd,get_version_tx_buf,ARRAY_SIZE(get_version_tx_buf));	
		usleep(milliSecDelay*1000);
	}
...
void sendCommandToTuner(uint8_t *packetBuf, uint8_t no_of_bytes)
{
	write(fd,packetBuf,no_of_bytes);
}
