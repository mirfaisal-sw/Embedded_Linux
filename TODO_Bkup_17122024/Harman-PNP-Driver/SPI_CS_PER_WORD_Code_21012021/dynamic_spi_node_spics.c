...
/*MIR*/
static struct spi_contoller_config_data my_spi_controller_data = {
	.turbo_mode = 0,
	.cs_per_word = 1, /*Toggle Chip Select line for every word transfer*/
};
...
static struct spi_board_info spi_device_info_pnp = {
	.max_speed_hz = 1500000, /* 1.5MHz */
	.bus_num = 1,
	.chip_select = 0,   /*MIR, changed 0 to 1 on 18-01-2022*/
	.mode = SPI_MODE_1,
...
static struct spi_contoller_config_data *my_debug;// = spi_device_info_pnp.controller_data;
...
static int spi_init(void)
{
	...
	case 3:
		...
		my_debug = spidev_info.controller_data;
		break;
	default:
		pr_err("module param mode not correct. Insert driver with correct mode\n");
		return -EPERM;
	}
	pr_info("bus_num:%d, mode:%d, chip_select:%d, max_speed_hz:%d =>, cs_per_word => %d !\n",
		spidev_info.bus_num, 
		spidev_info.mode,
		spidev_info.chip_select,
		spidev_info.max_speed_hz,
		my_debug->cs_per_word);
