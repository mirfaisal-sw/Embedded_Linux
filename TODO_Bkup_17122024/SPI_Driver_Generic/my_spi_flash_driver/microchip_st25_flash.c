...
static int send_write_data(void *data, size_t count)
{
	...
	res = spi_xchange(mirspi->spi, mirspi->tx_buf, NULL,
			mirspi->txlen);
	if (res) {
		dev_dbg(&mirspi->spi->dev,"PNP_Dabplugin: xchange failed[%d]\n", res);
		goto ret;
	}
...
static int mir_spi_probe(struct spi_device *spi)
{
	...
	//mutex_init(&mirspi->rlock);
	//mutex_init(&mirspi->wlock);
	//mutex_init(&mirspi->txrx_lck);
	INIT_LIST_HEAD(&mirspi->device_entry);
	init_waitqueue_head(&mirspi->wq_txrx);
