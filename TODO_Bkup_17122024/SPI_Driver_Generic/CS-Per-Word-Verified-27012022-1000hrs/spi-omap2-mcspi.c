...
static void omap2_mcspi_set_cs(struct spi_device *spi, bool enable)
{
	...
	{
		cnt++;
		pr_info("##### MIR CS toggled 20012022-1540- %d times",cnt);
	}
...
omap2_mcspi_txrx_pio(struct spi_device *spi, struct spi_transfer *xfer)
{
	...
	if (word_len <= 8) {
		...
		do {
			...
			//pr_info("MIR, spi->cs_gpio be - %d\n",spi->cs_gpio);
			//if (gpio_is_valid(spi->cs_gpio)){ /*I am getting spi->cs_gpio = -2 i.e. invalid*/
				omap2_mcspi_set_cs(spi, spi->mode & SPI_CS_HIGH); /*Activate CS*/
			//	pr_info("MIR, GPIO - %d is valid\n",(spi->cs_gpio));
			//}
				    ...
				    struct spi_transfer *t)
{
	...
	u8 				cs_per_word = 0; /*Temp Added by MIR*/
	mcspi = spi_master_get_devdata(master);
	mcspi_dma = mcspi->dma_channels + spi->chip_select;
	cs = spi->controller_state;
	...
	if(cs_per_word)/*Enabled by MIR*/
	{
		pr_info("MIR : CS per word is selected\n");
